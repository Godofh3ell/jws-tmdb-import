 ...
 * Init Streamvid TMDB.
 *
 * @package Streamvid/Integrations
 */

defined( 'ABSPATH' ) || exit;

include_once JWS_TMDB_IMPORT_ABSPATH . 'integrations/class-streamvid-tmdb-configuration.php';
include_once JWS_TMDB_IMPORT_ABSPATH . 'integrations/class-streamvid-tmdb-api-configuration.php';

class Streamvid_TMDB {
    ...
    public function __construct( $config = null ) {
        ...
        if ( ! $this->_loadConfig() ) {
            echo __( 'Unable to read configuration, verify that the API key is valid', 'jws' );
            exit;
        }
    }
    ...
    private function setConfig($config) {
        $this->config = new Streamvid_TMDB_Configuration( $config );
    }
    ...
    private function _loadConfig() {
        $this->_apiconfiguration = new Streamvid_TMDB_API_Configuration( $this->_call( 'configuration' ) );
     ...
     *  @return Movies Video[]
     */
    public function getVideoMovies($idMovie) {

        $movies_videos = array();

        $result = $this->_call( "movie/$idMovie/videos");

        if( ! empty( $result['results'] ) ) {
            $movies_videos = $result['results'];
        }
     ...
     *  @return Movies Video[]
     */
    public function getVideoTvhows($idTvshow) {

        $tv_videos = array();

        $result = $this->_call( "tv/$idTvshow/videos");

        if( ! empty( $result['results'] ) ) {
            $tv_videos = $result['results'];
        }
